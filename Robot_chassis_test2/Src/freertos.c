...
//  osThreadDef(vUser_Task, User_Task, osPriorityBelowNormal, 0, 128);
//  vUser_TaskHandle = osThreadCreate(osThread(vUser_Task), NULL);

  /* definition and creation of vStartTask */
  osThreadDef(vStartTask, StartTask, osPriorityLow, 0, 128);
  vStartTaskHandle = osThreadCreate(osThread(vStartTask), NULL);
...
/**
* @brief Function implementing the vStartTask thread.
* @param argument: Not used
* @retval None
*/
...
//  vUser_TaskHandle = osThreadCreate(osThread(vUser_Task), NULL);
	...
	vTaskDelete(vStartTaskHandle); //删除开始任务
